...
function buildRails(): Rail[] {
  ...
  for (const info of COUNTRIES) {
    ...
    // and `SUPPORTED_CURRENCIES` all filter on `collect`, so a payout-only row
     ...
     * already denominated in, and a currency added without checking it against
     * the account's own list is a checkout that fails at the last step.
     */
    if (!info.restricted) {
      rails.push({
        method: 'wallet',
 ...
 * of mobile money in every market, by accident, on a list nobody thought to
 * update. An enum's worth of cases belongs here in full.
 */
...
export function payoutRoute(country: Country, currency: Currency): PayoutRoute {
  ...
  if (info.paypalPayout) {
    return {
      ...
      blocked: false,
      verified: RAILS_VERIFIED,
      reason: `Paid in ${currency} to a PayPal account in ${info.name}. ` +
        'Neither local rail reaches this market, so the seller is paid into ' +
        'the PayPal account they register rather than to a bank.',
    }
  }
  ...
  return {
    ...
    blocked: true,
    verified: false,
    reason: `PayHold cannot send money to ${info.name} yet. No rail is ` +
